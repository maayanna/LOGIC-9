...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        if is_binary(new_formula.root):
            ...
            for key in relations_instantiation_map:
                my_str = relations_instantiation_map[key].__repr__()
                if str(var) in my_str:
                    if "A" in my_str or "E" in my_str:
                        continue
                    else:
                        raise Schema.BoundVariableError(str(var), key)
            prds = Schema._instantiate_helper(new_formula.predicate, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
            return  Formula(new_formula.root, str(var), prds)

        elif is_relation(new_formula.root):

            if new_formula.root in relations_instantiation_map:
                if not new_formula.arguments and "_" not in relations_instantiation_map[new_formula.root].__repr__():
                    return relations_instantiation_map[new_formula.root]
                elif not new_formula.arguments and "_" in relations_instantiation_map[new_formula.root].__repr__():
                    raise Schema.BoundVariableError("", new_formula.root)
                return (relations_instantiation_map[new_formula.root]).substitute({'_' : new_formula.arguments[0].substitute(constants_and_variables_instantiation_map, bound_variables)}, bound_variables)

        return new_formula.substitute(constants_and_variables_instantiation_map, bound_variables)
